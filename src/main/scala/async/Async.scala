...
object Async {
  ...
  /**
    ...
    */
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] = eventuallyX recover { case _ : Exception => -1 }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    makeAsyncComputation() recoverWith {
      case _: Throwable if maxAttempts > 1 => insist(makeAsyncComputation, maxAttempts - 1)
    }
  }
}
